...
@section('header')
    <h2 class="font-semibold text-xl text-text leading-tight">
        <a href="{{ route('books.index') }}" class="text-primary hover:text-primary-light">Livres</a> / Modifier "{{ $book->title }}"
    </h2>
@endsection
                ...
                <div class="p-6 bg-background-light border-b border-background-dark">
                    <form method="POST" action="{{ route('books.update', $book) }}" enctype="multipart/form-data">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <!-- Image de couverture -->
                            <div class="md:col-span-2">
                                <label class="block text-sm font-medium text-text-light">Image de couverture actuelle</label>
                                @if($book->cover)
                                    <img src="{{ Storage::url($book->cover) }}" alt="Couverture actuelle" class="h-32 mt-2 rounded border border-background-dark">
                                @else
                                    <p class="mt-2 text-sm text-text-light">Aucune image de couverture</p>
                                @endif
                                
                                <label for="cover" class="block mt-4 text-sm font-medium text-text-light">Nouvelle image</label>
                            ...
                            <div class="md:col-span-2">
                                ...
                                <textarea name="summary" id="summary" rows="4"
                                          class="mt-1 block w-full rounded-md border-background-dark shadow-sm focus:border-primary focus:ring focus:ring-primary focus:ring-opacity-50 bg-background text-text">{{ old('summary', $book->summary) }}</textarea>
                                @error('summary')
                                    <p class="mt-1 text-sm text-accent">{{ $message }}</p>
                                @enderror
                            </div>
